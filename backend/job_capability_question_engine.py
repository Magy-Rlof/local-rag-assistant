...
DOMAIN_SPECS = {
    "rag_retrieval": {
        ...
        "choice": [
            ...
            (
                ...
                "RAG 评估必须核对问题、召回片段和来源引用，不能只看生成结果。",
            ),
        ],
        "true_false": [
            ("RAG 回答必须能追溯到具体来源片段，否则很难判断答案是否来自岗位资料。", "正确", "来源引用是事实核验边界，不是装饰字段。"),
            ("只要大模型回答看起来合理，RAG 系统就可以忽略检索无结果和低相关片段。", "错误", "检索失败和低相关片段必须进入 fallback 或风险提示。"),
        ],
        "short": [
            ("请说明如何为一个 RAG 知识库设计召回质量验收方案。", ["准备代表性问题集", "检查召回片段和来源引用", "记录无结果、噪声和排序问题"]),
            ("请说明 RAG 中来源引用失真会带来哪些风险，以及如何修复。", ["答案不可核验", "可能引用错误岗位资料", "绑定片段 ID 并做输出校验"]),
        ],
    },
    "agent_orchestration": {
    ...
    "backend_api": {
        ...
        "short": [
            ("请说明岗位资料查询 API 应如何保证返回结果可被前端稳定消费。", ["固定响应 schema", "清晰错误码", "来源引用和日志可追踪"]),
